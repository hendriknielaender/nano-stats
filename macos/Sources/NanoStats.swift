...
private class MemoryStatusView: NSView {
    ...
    override init(frame: NSRect) {
        ...
        ram_label.textColor = NSColor.secondaryLabelColor
    ...
    func updatePercentage(_ percentage: Double) {
        percentage_label.stringValue = String(format: "%.1f%%", percentage)
    }
}
...
@objc public final class NanoStatsApp: NSObject, NSMenuDelegate {
    ...
    private let status_item_width: CGFloat = 60.0
    ...
    @objc public init(withTitle title: String) {
        ...
        if let button = self.status_item.button {
            ...
            let custom_view = MemoryStatusView(frame: NSRect(x: 0, y: 0, width: status_item_width, height: 22))
    ...
    public func menuWillOpen(_ menu: NSMenu) {
        ...
        if let breakdown = Self.fetch_system_memory_breakdown() {
            add_disabled_menu_item(title: "Total RAM: \(formatter.string(fromByteCount: Int64(breakdown.total_bytes)))")
            menu.addItem(NSMenuItem.separator())
            add_disabled_menu_item(title: "Active: \(formatter.string(fromByteCount: Int64(breakdown.active_bytes)))")
            add_disabled_menu_item(title: "Wired: \(formatter.string(fromByteCount: Int64(breakdown.wired_bytes)))")
            add_disabled_menu_item(title: "Inactive: \(formatter.string(fromByteCount: Int64(breakdown.inactive_bytes)))")
            add_disabled_menu_item(title: "Compressed: \(formatter.string(fromByteCount: Int64(breakdown.compressed_bytes)))")
            add_disabled_menu_item(title: "Used (A+I+W): \(formatter.string(fromByteCount: Int64(breakdown.used_bytes)))")
            add_disabled_menu_item(title: "Free: \(formatter.string(fromByteCount: Int64(breakdown.free_bytes)))")
        } else {
    ...
    private func add_quit_item(to menu: NSMenu) {
        let quit_item = NSMenuItem(title: "Quit NanoStats", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    private static func fetch_system_memory_breakdown() -> SystemMemoryBreakdown? {
        ...
        guard result == KERN_SUCCESS, count == host_vm_info64_count else {
            print("Error/Warning: host_statistics64 failed or returned unexpected count. Result: \(result), Count: \(count)")
            return nil
        }
        
        let page_size = UInt64(vm_kernel_page_size)
        guard page_size > 0 else { assertionFailure("vm_kernel_page_size is zero"); return nil }
    ...
    private static func fetch_total_physical_memory_bytes() -> UInt64 {
        ...
        guard result == 0, size == MemoryLayout<UInt64>.size, physical_memory > 0 else {
            print("Error fetching/validating hw.memsize. Result: \(result), Size: \(size), Errno: \(errno)")
            return 0
        }
    ...
    private static func fetch_top_memory_processes(limit: Int, total_physical_memory: UInt64) -> [ProcessDetails] {
        ...
        for i in 0..<process_count {
            ...
            let percentage = (total_physical_memory > 0) ? 
